...
public class ConstraintRegistrar {
    ...
    private static ResourceKey<SudokuConstraint> constraint(String name) {
        return ExperimentalRegistrars.dataKey(ExperimentalWorkshopRegistries.SUDOKU_CONSTRAINT_KEY, name);
    }
    ...
    public static class Tags {
        ...
        public static void constraintTags(Function<TriFunction<ExistingFileHelper, CompletableFuture<HolderLookup.Provider>, PackOutput, TagsProvider<SudokuConstraint>>, TagsProvider<SudokuConstraint>> factory) {
            tagProvider(ExperimentalWorkshopRegistries.SUDOKU_CONSTRAINT_KEY, factory, (registries, tag) -> {
        ...
        private static TagKey<SudokuConstraint> constraint(String name) {
            return tagKey(ExperimentalWorkshopRegistries.SUDOKU_CONSTRAINT_KEY, name);
        }

        private static <T> TagKey<T> tagKey(ResourceKey<? extends Registry<T>> registryKey, String name) {
            return TagKey.create(registryKey, AshsWorkshop.fromMod(name));
        }

        private static <T> void tagProvider(ResourceKey<? extends Registry<T>> registryKey, Function<TriFunction<ExistingFileHelper, CompletableFuture<HolderLookup.Provider>, PackOutput, TagsProvider<T>>, TagsProvider<T>> factory, BiConsumer<HolderLookup.Provider, Function<TagKey<T>, TagsProvider.TagAppender<T>>> addTags) {
